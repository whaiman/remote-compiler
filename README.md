...
**On the initial boot**, the server drops a `config/server.config.yaml` file in the execution directory containing cryptographically secure credentials.
...
On the local machine, trigger a dummy run to generate `config/client.config.yaml` or scaffold it yourself.
...
2. Bind the `auth_token` derived from the `config/server.config.yaml`.
...
> [!IMPORTANT]
> Create your config from the example before starting the container:
>
> ```bash
> cp config/server.config.yaml.example config/server.config.yaml
> # Edit config/server.config.yaml - at minimum, set a strong auth_token
> ```

```bash
# Run via Makefile (mounts config/ from the current directory)
make docker-run-server

# Or manually - mount the entire config/ directory into the container
docker run -d \
  --name rgcc-server \
  -p 4444:4444 \
...
The server will be reachable at `http://<host-ip>:4444`.
...
```text
Installation:
  install-client    Install client locally (-e .[client])
  install-server    Install server locally (-e .[server])
  install-dev       Install all dev dependencies

Development:
  dev-client        Run a dry-run client build (sample/main.cpp)
  dev-server        Run server in reload mode (127.0.0.1:4444)

Quality:
  test              Run pytest + lazy import checks
...
- **Token Storage**: `auth_token` is stored in plain text in `config/`. Ensure this directory has `chmod 700` permissions.
